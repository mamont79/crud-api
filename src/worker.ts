...
const sendResult = () => {
  parentPort!.on('message', (num) => {
    ...
    mamontServer.listen(treadServer, () => {
      console.log(
        `Server is running on port ${treadServer}. Use http://localhost:${treadServer}`
      );
    });
    parentPort!.postMessage(mamontServer);
  });
};
